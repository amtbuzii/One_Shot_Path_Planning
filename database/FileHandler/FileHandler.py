...
def save_graph(field: Field) -> None:
    from shapely import geometry
    import matplotlib.pyplot as plt
    import matplotlib.patches as Polygon
    fig = plt.figure()
    ax = fig.add_subplot(1, 1, 1)
    ax.set_xlim([0, constant.FIELD_SIZE])
    ax.set_ylim([0, constant.FIELD_SIZE])
    for ii in range(len(field.polygons)):
        poly = geometry.Polygon([[p.x, p.y] for p in field.polygons[ii]])
        x, y = poly.exterior.xy
        ax.plot(x, y, color = "black")
        ax.fill(x, y, color = "black")

    plt.savefig(fname = "fig.png", bbox_inches = 'tight')
    plt.show()
