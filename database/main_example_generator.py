...
class App(customtkinter.CTk):
    def __init__(self):
        ...
        # creatring masks
        img = Image.new('L', (int(self.field_size), int(self.field_size)), 0)
        mask = np.array(img).flatten()
        self.start_point_mask = mask
        self.end_point_mask = mask
        self.path_mask = mask
    ...
    def run_example_generator(self, index: int = 8,  # optimal
                              num_example_to_save: int = 0,
                              num_of_sets: int = 1,
                              save_every: int = 10000):

        run_seed = np.random.randint(0, 600)

        for ii in range(num_of_sets):
            print(ii)
            self.seed = np.random.randint(0, 600)
            self.start_point = (np.random.randint(self.field_size), np.random.randint(self.field_size))
            self.end_point = (np.random.randint(self.field_size), np.random.randint(self.field_size))
            ...
            if index not in [1, 2]:
                if output[0] == None:
                    output = "Sorry, No path found, try another algorithm."
                else:
                    output = "Length = {:.3f}, Runtime = {:.3f}".format(output[0], output[1])
                self.change_info(output)

            self.start_point_mask = np.concatenate((self.start_point_mask, masks.start_point_mask.flatten()))
            self.end_point_mask = np.concatenate((self.end_point_mask, masks.end_point_mask.flatten()))
            self.path_mask = np.concatenate((self.path_mask, masks.path_mask.flatten()))
            self.polygon_mask = np.concatenate((self.polygon_mask, masks.polygon_mask.flatten()))
            self.run_time.append(run_time)

            if ii < num_example_to_save:
                p = os.path.join('exmples_of_masks',
                                 str(run_seed) + '_' + str(self.field_size) + 'X' + str(self.field_size))
                if not os.path.isdir(p):
                    os.mkdir(p)
                p_dir = os.path.join(p, str(ii))
                os.mkdir(p_dir)

                resized_factor = 1
                if constant.FIELD_SIZE < 50:
                    resized_factor = 50
                elif constant.FIELD_SIZE >= 100:
                    resized_factor = 1
                else:
                    resized_factor = 10
                resized_start_point_mask = cv2.resize(masks.start_point_mask, (
                    constant.FIELD_SIZE * resized_factor, constant.FIELD_SIZE * resized_factor))
                plt.imsave(os.path.join(p_dir, "start_point_mask_" + str(ii) + ".png"), resized_start_point_mask)

                resized_end_point_mask = cv2.resize(masks.end_point_mask,
                                                    (constant.FIELD_SIZE * resized_factor,
                                                     constant.FIELD_SIZE * resized_factor))
                plt.imsave(os.path.join(p_dir, "end_point_mask_" + str(ii) + ".png"), resized_end_point_mask)

                resized_path_mask = cv2.resize(masks.path_mask,
                                               (constant.FIELD_SIZE * resized_factor,
                                                constant.FIELD_SIZE * resized_factor))
                plt.imsave(os.path.join(p_dir, "path_mask_" + str(ii) + ".png"), resized_path_mask)

                resized_polygon_mask = cv2.resize(masks.polygon_mask,
                                                  (constant.FIELD_SIZE * resized_factor,
                                                   constant.FIELD_SIZE * resized_factor))
                plt.imsave(os.path.join(p_dir, "polygon_mask_" + str(ii) + ".png"), resized_polygon_mask)

                sum_mask = 50 * resized_polygon_mask + 200 * resized_path_mask
                resized_sum_mask = cv2.resize(sum_mask,
                                              (constant.FIELD_SIZE * 50, constant.FIELD_SIZE * 50))
                plt.imsave(os.path.join(p_dir, "sum_mask_" + str(ii) + ".png"), resized_sum_mask)

                # sum_mask = 50 * masks.polygon_mask + 200 * masks.path_mask
                # plt.imshow(sum_mask)
                # plt.show()

            if (ii + 1) % save_every == 0:
                num_of_examples_to_save = ii + 1
                self.save_data(num_of_examples_to_save, run_seed)

        else:
            self.change_info("Please select a start point and an end point.")

        self.save_data(num_of_sets, run_seed)

    def save_data(self, num_of_examples_to_save, run_seed):

        p = os.path.join('dataset', str(num_of_examples_to_save) + '_' + str(self.field_size) + 'X' + str(self.field_size))
        if not os.path.isdir(p):
            os.mkdir(p)

        np.savetxt(os.path.join(p, 's_maps' + '.dat'),
                   self.start_point_mask.reshape(num_of_examples_to_save + 1, self.field_size * self.field_size),
                   fmt='% 4d')

        np.savetxt(os.path.join(p, 'g_maps' + '.dat'),
                   self.end_point_mask.reshape(num_of_examples_to_save + 1, self.field_size * self.field_size),
                   fmt='% 4d')

        np.savetxt(os.path.join(p, 'outputs' + '.dat'),
                   self.path_mask.reshape(num_of_examples_to_save + 1, self.field_size * self.field_size),
                   fmt='% 4d')

        np.savetxt(os.path.join(p, 'inputs' + '.dat'),
                   self.polygon_mask.reshape(num_of_examples_to_save + 1, self.field_size * self.field_size),
                   fmt='% 4d')


        avg_run_time = sum(self.run_time) / len(self.run_time)
        np.savetxt(os.path.join(p,'run_time_' + str(self.field_size) + 'x' + str(self.field_size) + '_' + str(
            num_of_examples_to_save) + '_' + str(run_seed) + '.dat'), self.run_time)
        np.savetxt(os.path.join(p,'run_time_avg' + str(self.field_size) + 'x' + str(self.field_size) + '_' + str(
            num_of_examples_to_save) + '_' + str(run_seed) + '.dat'), [avg_run_time])
...
def algorithms(field_size, start_point, end_point, index, seed) -> tuple[float, float]:
    ...
    img = Image.new('L', (int(field_size), int(field_size)), 0)
    mask = np.array(img)
    run_masks = masks.Masks(start_point_mask = mask, end_point_mask = mask, path_mask = mask,
                            polygon_mask = mask)
    ...
    if index == 1:
        ...
        gc.draw_graph()
        return naive_length, naive_runtime, run_masks
    elif index == 5:
        ...
        # gc.draw_graph()
        start_point_mask = gc.create_start_point_mask()
        end_point_mask = gc.create_end_point_mask()
        path_mask = gc.create_path_mask()
        polygon_mask = gc.create_polygon_mask()
        run_masks = masks.Masks(start_point_mask = start_point_mask, end_point_mask = end_point_mask,
                                path_mask = path_mask,
                                polygon_mask = polygon_mask)

        return optimal_length, optimal_runtime, run_masks
        ...
        run_masks = masks.Masks(start_point_mask = start_point_mask, end_point_mask = end_point_mask,
                                path_mask = path_mask,
                                polygon_mask = polygon_mask)
        return greedy_length, greedy_runtime, run_masks
        ...
        run_masks = masks.Masks(start_point_mask = start_point_mask, end_point_mask = end_point_mask,
                                path_mask = path_mask,
                                polygon_mask = polygon_mask)
        return random_length, random_runtime, run_masks
        ...
        run_masks = masks.Masks(start_point_mask = start_point_mask, end_point_mask = end_point_mask,
                                path_mask = path_mask,
                                polygon_mask = polygon_mask)
        return dubins_length, dubins_runtime, run_masks

    elif index == 8:
        # Create Astar Graph and find the Shortest Path
        test_field.polygons = field_m.get_convexhull_polygons()
        gc = GraphCreator(test_field)
        gc.create_graph(graph_type = "AStar")
        start_point_mask = gc.create_start_point_mask()
        end_point_mask = gc.create_end_point_mask()
        path_mask = gc.create_path_mask()
        polygon_mask = gc.polygon_mask
        run_masks = masks.Masks(start_point_mask = start_point_mask, end_point_mask = end_point_mask,
                                path_mask = path_mask,
                                polygon_mask = polygon_mask)
        return gc.astar_path_length, gc._time_astar, run_masks
...
def main_run_example_generator():
    app = App()
    num_of_examples = 500000
    start_time = time.time()
    app.run_example_generator(index=8, num_example_to_save=0, num_of_sets=num_of_examples, save_every=10000)
    runtime = time.time() - start_time
    logging.info("runtime time: {}".format(runtime))
    print("creating " + str(num_of_examples) + "examples for world size " + str(FIELD_SIZE) + "X" + str(
        FIELD_SIZE) + " took {} minuts".format(runtime / 60))


if __name__ == "__main__":
    # main()
    main_run_example_generator()
