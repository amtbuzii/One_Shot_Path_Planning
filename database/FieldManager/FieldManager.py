...
class FieldManager:
    def __init__(
            ...
            start: Point = constant.DEFAULT_START,
            end: Point = constant.DEFAULT_END,
            seed: int = constant.DEFAULT_SEED,
    ):
        """
        ...
        if not self._start.check_valid(0, self._size) or not self._end.check_valid(
                0, self._size
        ):
    ...
    def _create_polygons(self) -> list[list]:
        ...
        for polygon in range(self._ice_num):
            # random center coordinate- not first or last pixel inorder not to split the world into foreign components
            buffer_from_side = 3
            temp_point = Point(
                random.randint(buffer_from_side, self._size - buffer_from_side),
                random.randint(buffer_from_side, self._size - buffer_from_side)
            )
            ...
            temp_rnd_point = self._random_points(
                center_point = temp_point, radius = temp_radius, dots = temp_dots, buffer_from_side = buffer_from_side
    ...
    def _random_points(
            self, center_point: Point, radius: float, dots: int, buffer_from_side: int = 0
    ) -> list[Point]:
        """
        ...
        for i in range(dots):
            while True:
                ...
                if _x * _x + _y * _y < (
                        radius * radius
                ):  # check correctness of the coordinate
                    _x = min(center_point.x + _x, self._size - 1 - buffer_from_side)
                    _y = min(center_point.y + _y, self._size - 1 - buffer_from_side)
                    if buffer_from_side <= _x <= self._size - buffer_from_side and buffer_from_side <= _y <= self._size - buffer_from_side:
