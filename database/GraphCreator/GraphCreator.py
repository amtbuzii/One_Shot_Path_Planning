...
class GraphCreator:
    ...
    def create_graph(self, graph_type: str = "Naive") -> None:
        ...
        #         )
        #         print("in polygon.contains(shapely.geometry.Point(self._start))")
        #         graph_type == "Naive"
        #         PATH_IN_POLYGON =True

        self.polygon_mask = self.create_polygon_mask()
        PATH_IN_POLYGON = True
        while PATH_IN_POLYGON:
            self._start = (np.random.randint(self._field.size), np.random.randint(self._field.size))
            self._end = (np.random.randint(self._field.size), np.random.randint(self._field.size))
            PATH_IN_POLYGON = False
            if self.polygon_mask[self._start[0], self._start[1]] > 0 or self.polygon_mask[
                self._end[0], self._end[1]] > 0:
                PATH_IN_POLYGON = True
                print("changed start and end points")
            elif self._start[0] == self._end[0] and self._start[1] == self._end[1]:
                PATH_IN_POLYGON = True
                print("start point == end point")

        self._field.start = self._start
        self._field.end = self._end
        start_point_mask = self.create_start_point_mask()
        end_point_mask = self.create_end_point_mask()
        sum = 50 * start_point_mask + 50 * end_point_mask + 50 * self.polygon_mask
        if np.max(sum) > 50:
            if self._start[0] != self._end[0] or self._start[0] != self._end[0]:
                print("not good")
        ...
        if graph_type == "Optimal":
            self._union_convex
            if self._optimal_graph == False:
                graph_type == "Naive"
        elif graph_type == "Naive":
    ...
    def _astar_graph(self) -> None:
        start_time_astar = time.time()
        SIZE = int(self._field.size)
        G = nx.grid_2d_graph(SIZE, SIZE)
        pos = dict((n, n) for n in G.nodes())  # Dictionary of all positions
        labels = dict(((i, j), (i, j)) for i, j in G.nodes())

        Diagonals_Weight = np.sqrt(2)
        # add diagonals edges
        G.add_edges_from([
                             ((h, w), (h + 1, w + 1))
        ...
        try:
            self._astar_path = nx.astar_path(G, self._start, self._end)
            self._time_astar = time.time()-start_time_astar
            self.astar_path_length = nx.astar_path_length(G, self._start, self._end)
        except Exception as e:
            self._time_astar = time.time() - start_time_astar
            logging.info("Error: NetworkXNoPath end not reachable from start')", e)
            self._astar_path = None
            self.astar_path_length = -1

        self._short_path = self._astar_path
    ...
    def save_graph(self, save: bool = False, t: int = 0) -> None:
        pos = {point: point for point in self._graph.nodes}
        # add axis
        fig, ax = plt.subplots()
        plt.xlim(0, self._field.size)
        plt.ylim(0, self._field.size)
        my_dpi = 96
        plt.figure(figsize = (self._field.size / my_dpi, self._field.size / my_dpi), dpi = my_dpi)
        for i, p in enumerate(self._polygons):
            polygon1 = Polygon(p)
            x, y = polygon1.exterior.xy
        ...
        plt.savefig("fig_with_path.png", bbox_inches = 'tight', pad_inches = -0.1)
        ...
        import cv2
        img = cv2.imread("fig_with_path.png", 2)
        ret, bw_img = cv2.threshold(img, 127, 255, cv2.THRESH_BINARY)
        # converting to its binary form
        bw = cv2.threshold(img, 127, 255, cv2.THRESH_BINARY)
        cv2.imwrite("fig_with_path_binary.png", img)
        resized = cv2.resize(img, (constant.FIELD_SIZE, constant.FIELD_SIZE))
        binary_resized = resized
        binary_resized[binary_resized > 0] = 1
        # cv2.imshow('image', binary_resized)
        # cv2.waitKey(0)

    def create_polygon_mask(self):
        img = Image.new('L', (int(self._field.size), int(self._field.size)), 0)
        mask = np.array(img)
        ...
        # img_dilation[mask > 0] = 0
        # plt.imshow(img_dilation)
        # plt.show()

        return img_dilation

    def create_start_point_mask(self):
        img = Image.new('L', (int(self._field.size), int(self._field.size)), 0)
        mask = np.array(img)
        mask[int(np.round(self._start[0])), int(np.round(self._start[1]))] = 1
        return mask

    def create_end_point_mask(self):
        img = Image.new('L', (int(self._field.size), int(self._field.size)), 0)
        mask = np.array(img)
        mask[int(np.round(self._end[0])), int(np.round(self._end[1]))] = 1
        return mask

    def create_path_mask(self):
        img = Image.new('L', (int(self._field.size), int(self._field.size)), 0)

        if self.astar_path_length > 0:
            # draw path
            path = self._short_path
            for ii in range(len(path) - 1):
                # xxx = [path[ii], path[ii + 1]]
                # print(xxx)
                ImageDraw.Draw(img).line([path[ii], path[ii + 1]], fill = 1)

        elif self._short_path is not None:
            # draw path
            path = self._short_path[1]
            for ii in range(len(path) - 1):
                # xxx = [path[ii], path[ii + 1]]
    ...
    def create_path_dilation_mask(self):
        # import cv2
        #
        mask_path = self.create_path_mask()
        plt.imshow(mask_path)
        plt.show()
        mask_NB = mask_path
        mask_NB[mask_NB > 0] = 255
        ksize = (4, 4)
        blur_mask = cv2.blur(mask_NB, ksize)
        plt.imshow(blur_mask)
        plt.show()

        # create polygon mask
        polygon_img = Image.new('L', (int(self._field.size), int(self._field.size)), 0)
        polygon_mask = np.array(polygon_img)

        for i, p in enumerate(self._polygons):
            ImageDraw.Draw(polygon_img).polygon(p, outline = 1, fill = 1)
            polygon_mask = np.array(polygon_img)
        plt.imshow(polygon_mask)
        plt.show()

        blur_mask[polygon_mask > 0] = 0
        plt.imshow(blur_mask)
        plt.show()

        kernel = np.ones((3, 3), np.uint8)
        img_dilation = cv2.dilate(mask_path, kernel, iterations = 1)
        img_dilation[polygon_mask > 0] = 0
        plt.imshow(img_dilation)
        plt.show()
