...
PATH = '../database/5K/generated_environments_chunk_1/'
...
threshold_results = np.zeros([len(THRESHOLDS), 5])
for idx, t in enumerate(THRESHOLDS):
    THRESHOLD = t
    time_to_predict = []
    for i in range(1, N_tries):
        data_reshape = x3d[i].reshape(1, N, N, 3)
        start_time_to_predict = time.time()
        prediction = model.predict(data_reshape)
        end_time_to_predict = time.time() - start_time_to_predict
        result[i][0:2] = plot_row_th(example_num=i, row=x3d[i], actual_output=y[i], predicted_output=prediction[0, :, :, 0]) #  return path_exists, actual_length
        #if result[i][0] > -1: # if path exist
        time_to_predict.append(end_time_to_predict)
    threshold_results[idx][0:4] = main(result, N_tries)
    if len(time_to_predict) > 0 :
        threshold_results[idx][4] = np.mean(time_to_predict)
    else:
        threshold_results[idx][4] = 0


threshold_results_df = pd.DataFrame(threshold_results, columns=["paths_found", "paths_not_found", "paths_are_equal", "paths_are_longer", "prediction_time"])
threshold_results_df.index = THRESHOLDS
threshold_results_df.to_csv(str(N)+'x'+str(N)+'_threshold_results.csv')
if PLOT:
